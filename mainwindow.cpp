...
void MainWindow::recoloring_chart_slot () //меняет цвет
{
    if (chart_is_open)
        chart_repaint (open_file_name, combobox_chart_type->currentText(), chbox_bw_chart->checkState());
}

void  MainWindow::type_change_slot()  //меняет тип графика 
{
    if (chart_is_open) {
        chart_repaint (open_file_name, combobox_chart_type->currentText(), chbox_bw_chart->checkState());
    }
...
void MainWindow::open_directory_slot() //открывает директорию
{
...
void MainWindow::print_chart_slot() //печатает графики
{
    if (chart_is_open){
